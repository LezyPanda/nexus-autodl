...
class NexusAutoDL:
    ...
    SECONDARY_FG_COLOR = "#888888" # Dim color for status bar and help text
    ...
    def _setup_ui(self):
        ...
        action_frame.pack(fill="x", expand=True, pady=(15, 0))
        status_frame = Frame(main_frame, bg=self.BG_COLOR)
        status_frame.pack(fill="x", expand=True, pady=(10,0))
        ...
        self.color_swatch.grid(row=0, column=1, padx=5)
    ...
    def _load_config(self):
        try:
            ...
            config = {}
        # The default for confidence is now 0.8
        self.confidence.set(config.get("confidence", 0.8))
        ...
        self.show_visual_feedback.set(config.get("show_visual_feedback", False))
        self.feedback_color.set(config.get("feedback_color", "#00FF00"))
    ...
    def _show_log_window(self):
        ...
        # New: Static header for hotkey instructions
        ...
        self.log_text_widget = Text(text_frame, height=20, width=100, wrap="word", bg=self.INPUT_BG_COLOR, fg=self.FG_COLOR, bd=0, highlightthickness=0)
        self.log_text_widget.pack(side="left", fill="both", expand=True)
        
        scrollbar = Scrollbar(text_frame, command=self.log_text_widget.yview, bg=self.BG_COLOR, troughcolor=self.INPUT_BG_COLOR, bd=0, activebackground=self.BUTTON_ACTIVE_BG_COLOR)
